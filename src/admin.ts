// 管理后台 - API Key 管理界面

interface ApiKeyInfo {
  keyPrefix: string;
  name: string;
  createdAt: string;
  expiresAt: string | null;
  lastUsed: string | null;
  requestCount: number;
  remainingDays: number | null;
}

interface ApiResponse<T> {
  success: boolean;
  data?: T;
  error?: string;
  message?: string;
}

export function initAdminApp(): void {
  const app = document.getElementById('app');

  if (!app) {
    console.error('App element not found');
    return;
  }

  app.innerHTML = `
    <div class="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      <!-- Header -->
      <header class="border-b border-slate-700/50 bg-slate-900/80 backdrop-blur-sm sticky top-0 z-10">
        <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center">
              <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
              </svg>
            </div>
            <div>
              <h1 class="text-xl font-bold">管理后台</h1>
              <p class="text-xs text-slate-400">API Key 管理</p>
            </div>
          </div>
          <div class="flex items-center gap-4">
            <a href="/" class="text-sm text-slate-400 hover:text-white transition-colors flex items-center gap-2">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/>
              </svg>
              返回首页
            </a>
          </div>
        </div>
      </header>

      <!-- Main Content -->
      <main class="max-w-6xl mx-auto px-6 py-12">
        <!-- Stats Cards -->
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div class="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50 p-6">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm text-slate-400">总 Keys</p>
                <p id="stat-total" class="text-3xl font-bold mt-1">0</p>
              </div>
              <div class="w-12 h-12 rounded-lg bg-blue-600/20 flex items-center justify-center">
                <svg class="w-6 h-6 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/>
                </svg>
              </div>
            </div>
          </div>

          <div class="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50 p-6">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm text-slate-400">活跃 Keys</p>
                <p id="stat-active" class="text-3xl font-bold mt-1 text-green-400">0</p>
              </div>
              <div class="w-12 h-12 rounded-lg bg-green-600/20 flex items-center justify-center">
                <svg class="w-6 h-6 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
              </div>
            </div>
          </div>

          <div class="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50 p-6">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm text-slate-400">即将过期</p>
                <p id="stat-expiring" class="text-3xl font-bold mt-1 text-yellow-400">0</p>
              </div>
              <div class="w-12 h-12 rounded-lg bg-yellow-600/20 flex items-center justify-center">
                <svg class="w-6 h-6 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
              </div>
            </div>
          </div>

          <div class="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50 p-6">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm text-slate-400">总请求量</p>
                <p id="stat-requests" class="text-3xl font-bold mt-1">0</p>
              </div>
              <div class="w-12 h-12 rounded-lg bg-purple-600/20 flex items-center justify-center">
                <svg class="w-6 h-6 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                </svg>
              </div>
            </div>
          </div>
        </div>

        <!-- Create New Key Section -->
        <div class="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50 p-6 mb-8">
          <h2 class="text-lg font-semibold mb-4">创建新的 API Key</h2>
          <div class="flex flex-wrap gap-4 items-end">
            <div class="flex-1 min-w-[200px]">
              <label class="block text-sm text-slate-400 mb-2">Key 名称</label>
              <input 
                type="text" 
                id="new-key-name"
                placeholder="例如: 我的应用"
                class="w-full px-4 py-3 rounded-lg bg-slate-900/50 border border-slate-600 text-white placeholder-slate-500 focus:outline-none focus:border-purple-500"
              />
            </div>
            <div class="w-48">
              <label class="block text-sm text-slate-400 mb-2">有效期（天）</label>
              <select 
                id="new-key-expires"
                class="w-full px-4 py-3 rounded-lg bg-slate-900/50 border border-slate-600 text-white focus:outline-none focus:border-purple-500"
              >
                <option value="7">7 天</option>
                <option value="30" selected>30 天</option>
                <option value="90">90 天</option>
                <option value="180">180 天</option>
                <option value="365">365 天</option>
                <option value="0">永不过期</option>
              </select>
            </div>
            <button 
              id="create-key-btn"
              class="px-6 py-3 rounded-lg bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-semibold transition-all flex items-center gap-2"
            >
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
              </svg>
              创建 Key
            </button>
          </div>

          <!-- New Key Display -->
          <div id="new-key-result" class="hidden mt-6 p-4 bg-green-900/20 border border-green-700/50 rounded-lg">
            <div class="flex items-center justify-between flex-wrap gap-4">
              <div class="flex-1 min-w-[300px]">
                <p class="text-sm text-green-400 mb-2">新创建的 Key（请妥善保管，只显示一次）</p>
                <div class="flex gap-2">
                  <input 
                    type="text" 
                    id="new-key-value"
                    readonly
                    class="flex-1 px-4 py-3 rounded-lg bg-slate-900 border border-green-500/50 text-green-400 font-mono text-sm"
                  />
                  <button id="copy-key-btn" class="px-4 py-3 rounded-lg bg-slate-700 hover:bg-slate-600 text-white transition-colors">
                    复制
                  </button>
                </div>
              </div>
              <div class="text-sm text-slate-400">
                <span id="new-key-expires-info"></span>
              </div>
            </div>
          </div>
        </div>

        <!-- Keys List -->
        <div class="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50 overflow-hidden">
          <div class="px-6 py-4 border-b border-slate-700/50 flex items-center justify-between">
            <h2 class="text-lg font-semibold">API Keys 列表</h2>
            <button 
              id="refresh-btn"
              class="px-4 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-white text-sm transition-colors flex items-center gap-2"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
              </svg>
              刷新
            </button>
          </div>

          <!-- Table Header -->
          <div class="grid grid-cols-12 gap-4 px-6 py-3 bg-slate-900/30 text-sm text-slate-400 font-medium">
            <div class="col-span-2">名称</div>
            <div class="col-span-2">Key 前缀</div>
            <div class="col-span-2">创建时间</div>
            <div class="col-span-2">过期时间</div>
            <div class="col-span-1 text-center">剩余</div>
            <div class="col-span-1 text-center">请求</div>
            <div class="col-span-2 text-center">操作</div>
          </div>

          <!-- Keys List Container -->
          <div id="keys-list" class="divide-y divide-slate-700/50">
            <div class="px-6 py-8 text-center text-slate-500">
              加载中...
            </div>
          </div>
        </div>
      </main>

      <!-- Renew Modal -->
      <div id="renew-modal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-slate-800 rounded-2xl border border-slate-700 w-full max-w-md">
          <div class="p-6 border-b border-slate-700">
            <h3 class="text-lg font-semibold">续期 API Key</h3>
            <p id="renew-key-name" class="text-sm text-slate-400 mt-1"></p>
          </div>
          <div class="p-6">
            <label class="block text-sm text-slate-400 mb-2">设置新的过期时间</label>
            <select 
              id="renew-expires"
              class="w-full px-4 py-3 rounded-lg bg-slate-900/50 border border-slate-600 text-white focus:outline-none focus:border-purple-500"
            >
              <option value="7">7 天后过期</option>
              <option value="30">30 天后过期</option>
              <option value="90">90 天后过期</option>
              <option value="180">180 天后过期</option>
              <option value="365">365 天后过期</option>
              <option value="0">永不过期</option>
            </select>
          </div>
          <div class="p-4 border-t border-slate-700 flex justify-end gap-3">
            <button id="cancel-renew-btn" class="px-6 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-white transition-colors">
              取消
            </button>
            <button id="confirm-renew-btn" class="px-6 py-2 rounded-lg bg-purple-600 hover:bg-purple-500 text-white transition-colors">
              确认续期
            </button>
          </div>
        </div>
      </div>

      <!-- Footer -->
      <footer class="border-t border-slate-700/50 mt-16">
        <div class="max-w-6xl mx-auto px-6 py-6 text-center text-sm text-slate-500">
          <p>WHOIS Lookup API - 管理后台</p>
        </div>
      </footer>
    </div>
  `;

  // Initialize
  loadKeys();
  initEventHandlers();
}

function initEventHandlers(): void {
  const createKeyBtn = document.getElementById('create-key-btn')!;
  const refreshBtn = document.getElementById('refresh-btn')!;
  const copyKeyBtn = document.getElementById('copy-key-btn')!;
  const renewModal = document.getElementById('renew-modal')!;
  const cancelRenewBtn = document.getElementById('cancel-renew-btn')!;
  const confirmRenewBtn = document.getElementById('confirm-renew-btn')!;

  // Create key
  createKeyBtn.addEventListener('click', createNewKey);

  // Refresh list
  refreshBtn.addEventListener('click', loadKeys);

  // Copy key
  copyKeyBtn.addEventListener('click', async () => {
    const keyInput = document.getElementById('new-key-value') as HTMLInputElement;
    await navigator.clipboard.writeText(keyInput.value);
    showToast('已复制到剪贴板');
  });

  // Renew modal
  cancelRenewBtn.addEventListener('click', () => {
    renewModal.classList.add('hidden');
  });

  confirmRenewBtn.addEventListener('click', () => {
    const keyPrefix = confirmRenewBtn.dataset.keyPrefix;
    const expiresSelect = document.getElementById('renew-expires') as HTMLSelectElement;
    const expiresDays = parseInt(expiresSelect.value);

    if (keyPrefix) {
      renewKey(keyPrefix, expiresDays);
    }
  });
}

async function createNewKey(): Promise<void> {
  const nameInput = document.getElementById('new-key-name') as HTMLInputElement;
  const expiresSelect = document.getElementById('new-key-expires') as HTMLSelectElement;

  const name = nameInput.value.trim();
  const expiresInDays = parseInt(expiresSelect.value);

  if (!name) {
    showToast('请输入 Key 名称', 'error');
    return;
  }

  try {
    const response = await fetch('/api/keys', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name, expiresInDays }),
    });

    const data: ApiResponse<{ key: string; keyPrefix: string; expiresAt: string | null }> = await response.json();

    if (data.success && data.data) {
      const resultDiv = document.getElementById('new-key-result')!;
      const keyInput = document.getElementById('new-key-value') as HTMLInputElement;
      const expiresInfo = document.getElementById('new-key-expires-info')!;

      keyInput.value = data.data.key;
      expiresInfo.textContent = expiresInDays === 0 ? '永不过期' : `将于 ${formatDate(data.data.expiresAt!)} 到期`;
      resultDiv.classList.remove('hidden');

      nameInput.value = '';
      loadKeys();
      showToast('API Key 创建成功');
    } else {
      showToast(data.error || '创建失败', 'error');
    }
  } catch (error) {
    showToast('创建失败: ' + (error instanceof Error ? error.message : '未知错误'), 'error');
  }
}

async function loadKeys(): Promise<void> {
  const listContainer = document.getElementById('keys-list')!;
  const statTotal = document.getElementById('stat-total')!;
  const statActive = document.getElementById('stat-active')!;
  const statExpiring = document.getElementById('stat-expiring')!;
  const statRequests = document.getElementById('stat-requests')!;

  try {
    const response = await fetch('/api/keys');
    const data: ApiResponse<ApiKeyInfo[]> = await response.json();

    if (data.success && data.data) {
      const keys = data.data;

      // Update stats
      statTotal.textContent = keys.length.toString();
      statActive.textContent = keys.filter(k => !isExpired(k.expiresAt)).length.toString();
      statExpiring.textContent = keys.filter(k => k.remainingDays !== null && k.remainingDays <= 7 && k.remainingDays > 0).length.toString();
      statRequests.textContent = keys.reduce((sum, k) => sum + k.requestCount, 0).toString();

      // Render list
      if (keys.length === 0) {
        listContainer.innerHTML = `
          <div class="px-6 py-12 text-center text-slate-500">
            <svg class="w-12 h-12 mx-auto mb-4 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/>
            </svg>
            <p>暂无 API Keys</p>
            <p class="text-sm mt-1">点击上方按钮创建第一个 Key</p>
          </div>
        `;
      } else {
        listContainer.innerHTML = keys.map(key => {
          const isKeyExpired = isExpired(key.expiresAt);
          const isExpiringSoon = key.remainingDays !== null && key.remainingDays <= 7 && key.remainingDays > 0;

          let statusColor = 'text-green-400';
          let statusText = key.remainingDays === null ? '永不过期' : `${key.remainingDays} 天`;

          if (isKeyExpired) {
            statusColor = 'text-red-400';
            statusText = '已过期';
          } else if (isExpiringSoon) {
            statusColor = 'text-yellow-400';
          }

          return `
            <div class="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-slate-700/20 transition-colors">
              <div class="col-span-2">
                <div class="font-medium">${escapeHtml(key.name)}</div>
              </div>
              <div class="col-span-2">
                <code class="text-sm text-slate-400 font-mono">${key.keyPrefix}***</code>
              </div>
              <div class="col-span-2 text-sm text-slate-400">
                ${formatDate(key.createdAt)}
              </div>
              <div class="col-span-2 text-sm ${isKeyExpired ? 'text-red-400' : 'text-slate-400'}">
                ${key.expiresAt ? formatDate(key.expiresAt) : '永不过期'}
              </div>
              <div class="col-span-1 text-center ${statusColor} font-medium">
                ${statusText}
              </div>
              <div class="col-span-1 text-center text-slate-400">
                ${key.requestCount}
              </div>
              <div class="col-span-2 flex items-center justify-center gap-2">
                <button 
                  class="renew-btn px-3 py-1 rounded text-xs bg-purple-600/20 text-purple-400 hover:bg-purple-600/40 transition-colors"
                  data-key="${key.keyPrefix}"
                  data-name="${escapeHtml(key.name)}"
                >
                  续期
                </button>
                <button 
                  class="delete-btn px-3 py-1 rounded text-xs bg-red-600/20 text-red-400 hover:bg-red-600/40 transition-colors"
                  data-key="${key.keyPrefix}"
                  data-name="${escapeHtml(key.name)}"
                >
                  删除
                </button>
              </div>
            </div>
          `;
        }).join('');

        // Add event handlers
        listContainer.querySelectorAll('.renew-btn').forEach(btn => {
          btn.addEventListener('click', (e) => {
            const target = e.target as HTMLElement;
            const keyPrefix = target.dataset.key!;
            const name = target.dataset.name!;
            openRenewModal(keyPrefix, name);
          });
        });

        listContainer.querySelectorAll('.delete-btn').forEach(btn => {
          btn.addEventListener('click', (e) => {
            const target = e.target as HTMLElement;
            const keyPrefix = target.dataset.key!;
            const name = target.dataset.name!;
            deleteKey(keyPrefix, name);
          });
        });
      }
    }
  } catch (error) {
    listContainer.innerHTML = `
      <div class="px-6 py-12 text-center text-red-400">
        加载失败: ${error instanceof Error ? error.message : '未知错误'}
      </div>
    `;
  }
}

function openRenewModal(keyPrefix: string, name: string): void {
  const modal = document.getElementById('renew-modal')!;
  const nameEl = document.getElementById('renew-key-name')!;
  const confirmBtn = document.getElementById('confirm-renew-btn')!;

  nameEl.textContent = name;
  confirmBtn.dataset.keyPrefix = keyPrefix;
  modal.classList.remove('hidden');
}

async function renewKey(keyPrefix: string, expiresInDays: number): Promise<void> {
  try {
    const response = await fetch(`/api/keys/${keyPrefix}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ expiresInDays }),
    });

    const data: ApiResponse<{ expiresAt: string | null }> = await response.json();

    if (data.success) {
      document.getElementById('renew-modal')!.classList.add('hidden');
      loadKeys();
      showToast('续期成功');
    } else {
      showToast(data.error || '续期失败', 'error');
    }
  } catch (error) {
    showToast('续期失败: ' + (error instanceof Error ? error.message : '未知错误'), 'error');
  }
}

async function deleteKey(keyPrefix: string, name: string): Promise<void> {
  if (!confirm(`确定要删除 "${name}" 吗？\n此操作不可恢复！`)) {
    return;
  }

  try {
    const response = await fetch(`/api/keys/${keyPrefix}`, { method: 'DELETE' });
    const data: ApiResponse<void> = await response.json();

    if (data.success) {
      loadKeys();
      showToast('删除成功');
    } else {
      showToast(data.error || '删除失败', 'error');
    }
  } catch (error) {
    showToast('删除失败: ' + (error instanceof Error ? error.message : '未知错误'), 'error');
  }
}

function isExpired(expiresAt: string | null): boolean {
  if (!expiresAt) return false;
  return new Date(expiresAt) < new Date();
}

function formatDate(dateStr: string): string {
  const date = new Date(dateStr);
  return date.toLocaleDateString('zh-CN', {
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
  });
}

function escapeHtml(text: string): string {
  const div = document.createElement('div');
  div.textContent = text;
  return div.innerHTML;
}

function showToast(message: string, type: 'success' | 'error' = 'success'): void {
  const toast = document.createElement('div');
  toast.className = `fixed bottom-4 right-4 px-6 py-3 rounded-lg shadow-lg z-50 transform transition-all duration-300 ${
    type === 'success' ? 'bg-green-600' : 'bg-red-600'
  } text-white`;
  toast.textContent = message;
  document.body.appendChild(toast);

  setTimeout(() => {
    toast.classList.add('opacity-0', 'translate-y-2');
    setTimeout(() => toast.remove(), 300);
  }, 3000);
}

// 自动初始化
document.addEventListener('DOMContentLoaded', () => {
  initAdminApp();
});
